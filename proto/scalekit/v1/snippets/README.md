...
This directory contains code samples for the ScaleKit API v1. The structure is organized by injection timing to clearly separate pre-mixin and post-mixin extensions.
...
├── pre-mixin/                          # Extensions applied to individual YAML files BEFORE swagger mixin
│   ├── global/                         # Global API configuration
│   │   └── info.extensions.yaml        # API info, security definitions, x-scalar-* extensions
│   ├── organizations/                  # Corresponds to organizations.proto
│   │   ├── CreateOrganization.snippets.md
│   │   ├── GetOrganization.snippets.md
│   │   └── UpdateOrganization.snippets.md
│   ├── users/                          # Corresponds to users.proto
│   │   └── CreateUser.snippets.md
│   └── [other services...]
└── post-mixin/                         # Extensions applied to combined JSON AFTER swagger mixin
    └── schemas/                        # Schema-level extensions
        ├── schema.Domain.extensions.yaml
        └── [other schemas...]
```
...
1. **Pre-mixin Phase**:
   - Code samples (`*.snippets.md`) are injected into individual Swagger YAML files
   - Operation extensions (`*.extensions.yaml`) are added to individual YAML files
   - Global configuration from `global/info.extensions.yaml` creates `_global.swagger.yaml`

2. **Swagger Mixin**:
   - All individual YAML files are combined into `scalekit.swagger.json`

3. **Post-mixin Phase**:
   - Schema extensions (`schema.*.extensions.yaml`) are applied to the combined JSON

4. **Final Step**:
   - Scalar CLI upgrades the OpenAPI spec from 2.0 to 3.1
...
Create a new markdown file in the correct location (under `pre-mixin/`):

```bash
mkdir -p proto/scalekit/v1/snippets/pre-mixin/{service_name}
touch proto/scalekit/v1/snippets/pre-mixin/{service_name}/{RPCMethodName}.snippets.md
```
...
```bash
python3 scripts/snippets/code_sample_extractor.py proto/scalekit/v1/snippets/pre-mixin/{service_name}/{RPCMethodName}.snippets.md
```
...
```bash
python3 scripts/snippets/inject_snippets_to_proto.py --snippet-file proto/scalekit/v1/snippets/pre-mixin/{service_name}/{RPCMethodName}.snippets.md
```
...
This directory is versioned alongside the proto files. When creating a new API version (e.g., v2), create a corresponding `proto/scalekit/v2/snippets/` directory with the same `pre-mixin/` and `post-mixin/` structure.

## Schema Extensions

Schema-level extensions are placed in `post-mixin/schemas/` and follow the naming convention:

- **File names**: `schema.{MessageName}.extensions.yaml`
- **Purpose**: Modify schema properties, add enum examples, x-enum-varnames, etc.
- **Applied**: After swagger mixin, before OpenAPI 3.1 upgrade
  ...
  schemas:
    Domain:
      properties:
        status:
          example: "active"
          x-enum-varnames: ["INACTIVE", "ACTIVE", "SUSPENDED"]
```
